...
--
-- Esta migration é deliberadamente idempotente: funciona tanto sobre
-- o banco remoto legado quanto em uma instalação nova.
-- ============================================================
...
begin
  ...
  update public.profiles set role = target_role where user_id = target_user_id;
  if not found then
    raise exception 'Membro não encontrado.' using errcode = 'P0002';
  end if;
  return true;
...
begin
  ...
  if exists (
    ...
    raise exception 'O grupo contém um atleta inexistente ou sem acesso ativo.' using errcode = '22023';
  end if;
...
create policy posts_update_own_active on public.posts
  for update to authenticated
  using (user_id = (select auth.uid()) and (select app_private.is_active_member()))
  with check (user_id = (select auth.uid()) and (select app_private.is_active_member()));
