...
> Atualização 16/08/2026: foi preparado um handoff para outra IA, sem implementar funcionalidades. O proprietário rejeitou o boneco geométrico, a divergência de anatomia app/PDF e as letras de `logo-on-light.png`; relatou erro segmentar e exigiu OCR revisável, pesquisa Tanita estruturada, anatomia científica com versões/biotipos, gestão de treinos por atleta/mesociclo, remoção do nível redundante de treino e correção da âncora do treinador. Administrador mantém paridade técnica com treinador; loja permanece pausada. Ler o documento `26` antes de executar qualquer próximo bloco. Produção funcional permanece na baseline `9aafa8f`.

> Atualização 15/08/2026: a avaliação privada foi elevada para **Avaliação Tetrapolar Segmentada**. O app possui mapa corporal com as 10 medidas de braços/pernas/tronco, equilíbrio bilateral e seis gráficos de evolução; o PDF agora tem três páginas com 15 resultados gerais, todos os segmentos, foto circular e histórico. Logo transparente, favicon oficial e rótulo `Meta` foram corrigidos. OCR, Supabase e loja permaneceram intactos. PR `#34`, merge `6a38acc`, CI `31894361903` e Vercel `dpl_DoAT7djHUS9JgJSBDZUP5Ew477ZS` concluídos. Consulte o documento `25`.

> Atualização 15/08/2026: modos técnicos e esportivos foram separados integralmente. Samuel usa Administrador/Aluno e Robson usa Treinador/Atleta, sem menus cruzados; apenas o seletor de modo conecta `/admin/*` e `/dashboard/*`. O modal de avaliação foi corrigido para Android com `100dvh`, safe areas e rolagem própria, validado em 390 × 844. O OCR agora é posicional para Healthy Edge Lite e recuperou todos os 14 campos gerais preenchidos da Foto 1 e 10/10 segmentos da Foto 3 na amostra real. Publicação concluída pela PR `#32`, merge `fa7ac1b`, CI `31887206903` e Vercel `dpl_CJLzvTXyAS9fffKpPuNXuWSZr3Au`. Pesquisa de exportação e decisões estão no documento `24`. Loja e Supabase não foram alterados.
