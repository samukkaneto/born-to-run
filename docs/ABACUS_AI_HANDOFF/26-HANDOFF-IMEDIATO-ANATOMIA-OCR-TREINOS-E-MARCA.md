...
- **Atualização do Bloco C (16/08): ilustração conforme a balança, não o IMC.** Regra definida pelo proprietário: a escolha da variante anatômica é feita primeiro pelo **percentual de gordura corporal da Tanita** (`body_fat_pct` + `physique_rating`), porque a balança é mais específica que o IMC — um fisiculturista com IMC de "obeso" e gordura baixa deve aparecer com a ilustração de boa forma, não obesa. Faixas de %G (base da tabela oficial Tanita, correspondência IMC↔%G de Gallagher et al.): homens <11% magro · 11–22% saudável · 22–35% maior volume; mulheres <16% magro · 16–30% saudável · 30%+ maior volume. Physique rating 8–9 (musculoso) força o retrato magro e 1–3 (obeso/sólido) força o de maior volume. O IMC entra apenas como **fallback** quando não há avaliação. A escolha manual salva no banco tem sempre a última palavra. Lógica centralizada em `lib/assessments/anatomy-assets.ts` (`classifyIllustrationBiotype` / `autoAnatomyAssetPath`), aplicada no `SegmentedBodyMap.tsx` (app) e na página 2 do `lib/assessments/pdf.ts`. O Bloco B (exportação Tanita em arquivo) foi **pausado por decisão do proprietário**: o Healthy Edge Lite não exporta CSV/XLSX e a licença Pro não foi aprovada para compra; o fluxo oficial é fotos + OCR + correção manual.
...
- **Autenticação e recuperação de senha (16/08): corrigidas.** PR #42 mesclado e publicado. A causa foi o fallback de `getSiteOrigin()` para `VERCEL_URL` quando `NEXT_PUBLIC_SITE_URL` não estava disponível: o link de recuperação podia ser criado com o alias de um deployment, separado do domínio em que o pedido havia começado, fazendo o callback perder a sessão PKCE e retornar ao login. Em produção, o fallback agora é o domínio canônico `https://equipeborntorun.com`; em previews, usa-se somente o host do próprio preview. O `Site URL` do Supabase foi confirmado como `https://equipeborntorun.com` e não foi alterado. A tela de login também deixou de usar “comunidade privada” e passou a comunicar “Movimento e saúde” / “Treinamento, movimento e saúde para todos”.
- **Layout das avaliações (16/08): concluído.** PR #50 mesclado e publicado. As faixas de referência agora ocupam a largura total do enquadramento e o painel preto do mapa corporal segmentado fica em uma segunda linha, também em largura total. Os dados de gordura/músculo continuam sobrepostos nas regiões correspondentes da anatomia; nenhuma regra de classificação, dado privado ou autorização foi alterada.
