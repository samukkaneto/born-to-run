...
> Atualização 16/08/2026 — **handoff documental, sem implementação funcional nova.** O proprietário rejeitou a silhueta geométrica da avaliação, a divergência entre anatomia do app/PDF e as letras da variante `logo-on-light.png`. Também relatou uma leitura segmentar absurda, exigiu revisão manual segura, pesquisa de CSV/XLS/XLSX Tanita, gerenciamento de treinos por atleta/mesociclo e remoção do nível Iniciante/Intermediário/Avançado somente dos treinos. O botão `Conheça o treinador` deve apontar diretamente para Robson. Loja continua pausada. A especificação integral para a próxima IA está no documento `26`; o estado publicado ainda é a revisão `9aafa8f`.

> Atualização 15/08/2026 — a área privada agora se chama **Avaliação Tetrapolar Segmentada** e possui mapa corporal com 10 leituras em braços, pernas e tronco, quatro balanços bilaterais e seis gráficos de evolução. O PDF foi reconstruído em três páginas A4 com 15 indicadores gerais, todos os segmentos, foto circular e histórico. Login/cadastro usam a variante oficial transparente para fundo claro sem sombra; favicon e rótulo `Meta` foram corrigidos. OCR, Supabase e loja não mudaram. Publicação: PR `#34`, merge `6a38acc`, CI da `main` `31894361903` e Vercel `dpl_DoAT7djHUS9JgJSBDZUP5Ew477ZS` `READY`. Consulte `25-AVALIACAO-TETRAPOLAR-PREMIUM-PDF-E-IDENTIDADE.md`.

> Atualização 15/08/2026 — Samuel e Robson agora possuem experiências de gestão e esportiva visualmente separadas: `/admin/*` não mistura perfil de atleta e `/dashboard/*` não mostra ferramentas administrativas; somente o seletor explícito de modo faz a troca. O modal Tanita foi corrigido com `100dvh`, áreas seguras e rolagem interna, validado em 390 × 844. O OCR passou a usar template posicional do Healthy Edge Lite: 14 indicadores gerais preenchidos na Foto 1 e 10/10 segmentos na Foto 3 foram recuperados na amostra real, com revisão humana obrigatória. As fontes passaram a ser locais após o primeiro CI revelar uma URL antiga da Oswald; o CI final ficou verde. Publicação: PR `#32`, merge `fa7ac1b`, CI da `main` `31887206903` e Vercel `dpl_CJLzvTXyAS9fffKpPuNXuWSZr3Au` `READY`. Não houve mudança de banco nem da loja. Consulte `24-MODOS-SEPARADOS-MOBILE-E-OCR-TANITA-CALIBRADO.md`.

> Atualização 15/08/2026 — correção de prescrição privada e Tanita completa aplicada no Supabase remoto e publicada pela PR `#30`, merge `694be8c`, CI da `main` `31882239549` e Vercel `dpl_DiEf7ec6fTdPqFji4AvX9NYZAM58` `READY`. Samuel (`admin/active`) e Robson Alves (`coach/active`) agora podem receber treinos no próprio modo atleta; o gestor escolhe o destinatário antes de criar; o atleta ganhou Hoje/Calendário/Mesociclo; a Tanita exige Foto 1/2/3 e apresenta 25 medidas, faixa traduzida, gráficos e segmentos; galeria não exige legenda ou descrição. Três migrations foram aplicadas. Gates: lint, TypeScript, 74 unitários, build de 40 rotas e matriz Playwright pública aprovada; pgTAP novo está versionado, mas o local não executou por ausência de Docker. A loja não foi alterada. Consulte `23-PRESCRICAO-PRIVADA-MESOCICLO-E-TANITA-3-FOTOS.md`.

> Atualização 15/08/2026 — o bloco de retomada foi implementado, validado e publicado pela PR `#28`, merge `7c5561b28051f6aad873f2ba56e77649ae052165`, CI da `main` `31871069506` e deployment Vercel `dpl_FqV47iK47bRQ1vLkk3AofS2A3vbA`. Foram entregues os modos Administrador/Aluno e Treinador/Atleta, funções técnicas temporárias para o administrador, metas pessoais com RLS exclusiva do atleta, carregamento resiliente de missões/conquistas, área privada de evolução, loja sem orientações internas/fornecedores visíveis e relatório PDF protegido. O Supabase remoto contém a migration `20260815043638_admin_training_private_goals`. Gates locais: lint, TypeScript, 70 unitários, build de 40 rotas, 20 asserções pgTAP e 28 cenários Playwright aprovados. Consulte `22-MODOS-METAS-EVOLUCAO-E-LOJA.md`.
