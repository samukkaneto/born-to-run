...
> Atualização 16/08/2026: o novo documento `26` é somente handoff e não altera código, banco ou produção. Portanto, nenhuma das tarefas nele descritas deve ser marcada como testada/concluída. O próximo executor deve gerar novas evidências para OCR campo a campo, revisão manual, anatomia app/PDF, filtros de treinos, migration de nível, logo original e âncora do treinador. Os gates do release `9aafa8f` continuam sendo apenas a baseline anterior.

> Atualização 15/08/2026: o bloco visual da Avaliação Tetrapolar passou `npm run check`, 76/76 unitários, build de 40 rotas e matriz Playwright 28/28 em Desktop Chrome/Pixel 7. A prévia específica foi inspecionada em 1440 × 1000 e 390 × 844, sem overflow ou erros de console. O PDF de demonstração possui 3 páginas A4 e 367.326 bytes; as páginas foram renderizadas a 150 DPI e inspecionadas. CI da PR `31894121724`, CI da `main` `31894361903` e produção `dpl_DoAT7djHUS9JgJSBDZUP5Ew477ZS` aprovados. Consulte `25-AVALIACAO-TETRAPOLAR-PREMIUM-PDF-E-IDENTIDADE.md`.

> Atualização 15/08/2026: o bloco de modos separados e Tanita calibrada passou TypeScript, ESLint direcionado/completo, 76/76 unitários, build Next.js 16.3 de 40 rotas e Playwright 28/28 em Desktop Chrome/Pixel 7. A amostra privada do Healthy Edge Lite recuperou todos os 14 campos gerais preenchidos e 10/10 segmentos; o campo de frequência cardíaca vazio não foi inventado. O modal foi medido separadamente em 390 × 844: título e pessoa avaliada visíveis no topo, `scrollTop = 0` na abertura e botão final totalmente acessível no limite inferior. CI final da PR `31887079146` e CI da `main` `31887206903` aprovados; produção `dpl_CJLzvTXyAS9fffKpPuNXuWSZr3Au` validada na revisão `fa7ac1b`. Consulte `24-MODOS-SEPARADOS-MOBILE-E-OCR-TANITA-CALIBRADO.md`.
